...
The composer stays open while a turn runs. A message sent mid-turn waits in a small tray on the composer's top edge and goes out when the running turn settles — one turn at a time, each with a fresh editor snapshot, so a queued ask always sees the edits made before it. A typed triage places each mid-turn message first: a refinement of the running work is steered into the live turn, an independent ask that cannot collide starts a parallel thread, and everything else — including whatever the call cannot place — waits in the tray, where the running agent can also park a message itself when it needs the finished result first. A row is placed against the tools the turn had run at the time, so the tray is placed again as the turn advances: the work a row was waiting on lands, and it folds in rather than waiting out the turn. The tray is the review surface: waiting rows can be reordered, rewritten, or removed, and the queue pauses and resumes from its header. While a row is being rewritten the list holds still — the row under edit waits, the others keep draining and cross out in place as they finish. Stopping a turn or hitting an error pauses the queue automatically, so the remaining asks hold for review. Waiting rows are saved with the thread on the machine: a reload or a trip to another project brings them back paused, ready to resume.
...
- **Watching leaves a record.** Contact sheets live in one conversation and the media trim drops the oldest of them, so the model writes each look down: a note-source call stores what a span showed — quoted on-screen text, what happens, where the cuts land — against its source seconds, on the asset, saved with the project. Notes come back in the editor snapshot as `observed`, in full from get-state, and on every watch result beside the spans still undescribed. A close note supersedes the coarse one over the same ground, and a watch of a source that still owes a note is refused, which is what makes an hour-long source workable: it is surveyed coarsely, described, then re-watched densely only where the work is.
...
The committed report at `evals/cut-chat.latest-report.json` is the baseline. The improvement loop: make one change — a model role, a prompt line, a tool description — run the eval, and keep the change only when pass rates hold and the medians improve; the new report commits with the change, so the baseline always describes what ships. This is how the complexity router and the batched-round cutting flow earned their way in. Run it with the dev server up: `npm run eval:cut-chat` (add `--matrix --runs 3` to compare model configs) and `npm run eval:cut-queue` for the mid-turn triage; the README beside the harness under the site's `scripts/lib/cut-eval/` folder covers the flags, the report schema, and how to add a case. Run `node scripts/check-chat-continuity.mjs /path/to/compiled-engine` to verify disconnect recovery, replay, and the native HTTP media boundary without tokens. The tab-less paths also have end-to-end scripts, which drive a real turn against a seeded cloud project through the headless session and through the queued turn job.
