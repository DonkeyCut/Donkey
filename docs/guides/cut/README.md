...
The client probes the engine's health endpoint (dedicated port first, dev server second) and remembers the winner. Browsers permission-gate a hosted page's calls to the local machine, so the page probes only when that raises no prompt — it is served by the engine, or the permission is already granted — and starts in the cloud otherwise. The engine grants the hosted origin cross-origin access, and only that origin. Nothing walls the app, because cloud editing always works. What the page will not do is fail quietly: a browser that has reached this Mac's engine before and cannot now — the app is closed, or the browser is blocking the connection — gets a banner it cannot dismiss, since its local projects are the ones that will not open, and the banner opens the recovery steps in a dialog with the button that fires the browser's permission prompt. Probing continues behind it, so starting the app clears the banner. Engine updates ride the Donkey app's own auto-updater, so the client never prompts to update.
