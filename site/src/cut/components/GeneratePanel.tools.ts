...
export const VIDEO_GEN_TOOLS = [
  {
    ...
    inputSchema: obj({
      ...
      resolution: { type: "string", enum: [...geminiOmniResolutions], description: `Output size (default ${geminiOmniDefaultResolution}; 1080p and 4k are upscaled and cost more per second)` },
      duration_seconds: num(`Clip length in whole seconds, ${DURATION.min}–${DURATION.max}; omit to let the model pick`),
