...
/// The rollup the site's pipeline writes today, decoded by this build. The
/// fixture is generated by `npm run analytics:rollup-fixture` in site/ and
/// checked by the site's tests, so a shape change on the server fails here
/// before it reaches a phone.
@Suite struct AnalyticsContractTests {
    func fixture() throws -> Data {
        let url = try #require(Bundle.module.url(
            forResource: "analytics-rollup",
            withExtension: "json",
    ...
    @Test func decodesTheRollupThePipelineWrites() throws {
        let rollup = try AnalyticsRollup.decode(fixture())
        let summary = AnalyticsSummary(rollup: rollup)
        #expect(rollup.days == ["2026-08-30", "2026-08-31", "2026-09-01"])
    ...
    @Test func aMovedFieldIsNamed() throws {
        ...
        #expect(throws: AnalyticsError.unreadable("billing.canceling is not an array")) {
            try AnalyticsRollup.decode(data)
        }
    }
    ...
    @Test func aMissingFieldIsNamed() throws {
        ...
        json["days"] = nil
        let data = try JSONSerialization.data(withJSONObject: json)
        #expect(throws: AnalyticsError.unreadable("days is missing")) {
            try AnalyticsRollup.decode(data)
        }
    }
}
