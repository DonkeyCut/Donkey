...
@Suite struct CameraModelTests {
    ...
    @Test func audioMeterShowsByDefaultIsRememberedAndOnlyReadsWhileShown() {
        ...
        model.toggleAudioMeter()
        #expect(model.showsAudioMeter)
        #expect(controller.recorded == ["meter false", "meter true"])
    }
...
@Suite struct AnalyticsSummaryTests {
    // Three days, one of them never extracted, as the server folds them.
    func document(billing: AnalyticsSummaryDocument.Billing? = nil) -> AnalyticsSummaryDocument {
        AnalyticsSummaryDocument(
            generatedAt: "2026-08-21T04:00:00.000Z",
            series: [
                .init(day: "2026-08-18", active: 1, working: 1, signups: 0, totalRegistered: 1),
                .init(day: "2026-08-19", active: nil, working: nil, signups: 1, totalRegistered: 2),
                .init(day: "2026-08-20", active: 2, working: 1, signups: 0, totalRegistered: 2),
            ],
            registered: 2,
            signups7d: 1,
            signupsWindow: 1,
            ...
            active7d: 2,
            activePrior7d: 1,
            balanceMicros: "3500000",
            missing: ["2026-08-19"],
            billing: billing
        )
    }

    @Test func drawsThePointsTheServerFolded() {
        let summary = AnalyticsSummary(document: document())
        ...
        #expect(summary.weekDeltaPercent == 100)
        #expect(summary.balanceDollars == 3.5)
    }

    @Test func billingChartsOnItsOwnWindow() {
        // The billing window runs a day past the activity window, and every
        // day of it counts: today's money is in the total the day it lands.
        let billing = AnalyticsSummaryDocument.Billing(
            days: ["2026-08-18", "2026-08-19", "2026-08-20", "2026-08-21"],
            subscribers: 4,
            canceling: [.init(subscriptionId: "sub_1")],
            funded: 6,
            ...
            revenue: [
                ...
                .init(proMicros: "30000000", topupMicros: "0"),
            ]
        )
        let summary = AnalyticsSummary(document: document(billing: billing))
        #expect(summary.subscribers == 4)
        #expect(summary.canceling == 1)
        #expect(summary.fundedDollars == 120)
        #expect(summary.revenue.count == 4)
        #expect(summary.revenueDollars == 45)
        #expect(summary.revenue[0].proDollars == 10)
        #expect(summary.revenue[1].topupDollars == 5)
        #expect(summary.revenue[3].revenueDollars == 30)
    }

    @Test func billinglessSummaryLeavesRevenueUnknown() {
        let summary = AnalyticsSummary(document: document())
        #expect(summary.revenueDollars == nil)
        #expect(summary.revenue.isEmpty)
        #expect(summary.subscribers == nil)
    }

    @Test func aWeekWithoutABaselineHasNoDelta() {
        var doc = document()
        doc.activePrior7d = nil
        #expect(AnalyticsSummary(document: doc).weekDeltaPercent == nil)
    }
}

/// The summary the site's API serves today, decoded by this build. The fixture
/// is generated by `npm run analytics:summary-fixture` in site/ and checked by
/// the site's tests, so a shape change on the server fails here before it
/// reaches a phone.
@Suite struct AnalyticsContractTests {
    func fixture() throws -> Data {
        let url = try #require(Bundle.module.url(
            forResource: "analytics-summary",
            withExtension: "json",
    ...
    @Test func decodesTheSummaryTheApiServes() throws {
        let summary = try AnalyticsSummary(document: AnalyticsSummaryDocument.decode(fixture()))
        #expect(summary.registered == 3)
        #expect(summary.points.map(\.active) == [2, nil, 3])
        #expect(summary.points.map(\.working) == [2, nil, 1])
        #expect(summary.points.map(\.signups) == [0, 1, 0])
        #expect(summary.points.map(\.totalRegistered) == [2, 3, 3])
        #expect(summary.missingDayCount == 1)
        #expect(summary.balanceDollars == 3.5)
        #expect(summary.subscribers == 2)
        #expect(summary.canceling == 1)
        #expect(summary.funded == 2)
        #expect(summary.fundedDollars == 25)
        #expect(summary.revenue.count == 4)
        #expect(summary.revenue.map(\.proDollars) == [20, 0, 0, 0])
        #expect(summary.revenue.map(\.topupDollars) == [0, 0, 5, 0])
        #expect(summary.revenueDollars == 25)
        #expect(summary.generatedAt != nil)
    }

    /// Nobody's email or balance rides in what the phone reads.
    @Test func carriesNoAccountRows() throws {
        let json = try #require(try JSONSerialization.jsonObject(with: fixture()) as? [String: Any])
        #expect(json["users"] == nil)
    }

    @Test func decodesTheUsersPageTheApiServes() throws {
        let url = try #require(Bundle.module.url(
            forResource: "analytics-users",
            withExtension: "json",
            subdirectory: "Fixtures"
        ))
        let page = try AnalyticsUsersPage.decode(try Data(contentsOf: url))
        #expect(page.users.map(\.email) == ["p@x.test", "n@x.test", "s@x.test"])
        #expect(page.users.map(\.activeDays) == [2, 1, 2])
        #expect(page.users.last?.superUser == true)
        #expect(page.users.first?.fundedMicros == "20000000")
        #expect(page.total == 3)
        #expect(page.nextCursor == nil)
        #expect(page.pageSize == 50)
        #expect(page.sorts.map(\.id) == ["active", "recent", "joined", "paid"])
    }

    @Test func aMovedFieldIsNamed() throws {
        var json = try #require(try JSONSerialization.jsonObject(with: fixture()) as? [String: Any])
        var billing = try #require(json["billing"] as? [String: Any])
        billing["canceling"] = 3
        json["billing"] = billing
        let data = try JSONSerialization.data(withJSONObject: json)
        #expect(throws: AnalyticsError.unreadable("billing.canceling is not an array")) {
            try AnalyticsSummaryDocument.decode(data)
        }
    }

    @Test func aMovedScalarIsNamedInWords() throws {
        var json = try #require(try JSONSerialization.jsonObject(with: fixture()) as? [String: Any])
        var billing = try #require(json["billing"] as? [String: Any])
        billing["subscribers"] = "2"
        json["billing"] = billing
        let data = try JSONSerialization.data(withJSONObject: json)
        #expect(throws: AnalyticsError.unreadable("billing.subscribers is not a number")) {
            try AnalyticsSummaryDocument.decode(data)
        }
    }

    @Test func aMissingFieldIsNamed() throws {
        var json = try #require(try JSONSerialization.jsonObject(with: fixture()) as? [String: Any])
        json["series"] = nil
        let data = try JSONSerialization.data(withJSONObject: json)
        #expect(throws: AnalyticsError.unreadable("series is missing")) {
            try AnalyticsSummaryDocument.decode(data)
        }
    }
}
...
@Suite struct AnalyticsModelTests {
    final class Service: AnalyticsServicing, @unchecked Sendable {
        ...
        var calls = 0
        /// Pages keyed by the cursor asked for, and every (sort, cursor) asked.
        var pages: [Int: AnalyticsUsersPage] = [:]
        var pageCalls: [(sort: String, cursor: Int)] = []
        var pageFailure: (any Error)?
        ...
        func fetchAnalyticsUsers(sort: String, cursor: Int) async throws -> AnalyticsUsersPage {
            pageCalls.append((sort, cursor))
            if let pageFailure { throw pageFailure }
            return pages[cursor] ?? AnalyticsUsersPage(
                generatedAt: "2026-08-21T04:00:00.000Z",
                users: [],
                total: 0,
                nextCursor: nil,
    ...
    static func person(_ id: String) -> AnalyticsUsersPage.Person {
        .init(
            id: id,
            email: "\(id)@x.test",
            name: id,
    ...
    static func page(
        ...
        next: Int?,
        total: Int,
        generatedAt: String = "2026-08-21T04:00:00.000Z"
    ) -> AnalyticsUsersPage {
        AnalyticsUsersPage(
            generatedAt: generatedAt,
            users: ids.map(person),
            total: total,
            nextCursor: next,
            sorts: [.init(id: "active", label: "Most active"), .init(id: "joined", label: "Newest")],
            pageSize: 2
        )
    }

    func document() -> AnalyticsSummaryDocument {
        AnalyticsSummaryDocument(
            generatedAt: "2026-08-21T04:00:00.000Z",
            series: [.init(day: "2026-08-20", active: 1, working: 1, signups: 0, totalRegistered: 1)],
            registered: 1,
            signups7d: 0,
            signupsWindow: 0,
            activeYesterday: 1,
    ...
    @Test func aRetryOverAFailureSpinsAgain() async {
        ...
        let model = AnalyticsModel(service: service)
        await model.refresh()
        service.answer = .success(document())
        await model.refresh()
    ...
    // The list follows the cursor the API hands back, the same rule the web
    // dashboard's grid follows: ask for the first page, then the next.
    @Test func peopleArriveAPageAtATime() async {
        let service = Service()
        service.answer = .success(document())
        service.pages = [
            0: Self.page(["a", "b"], next: 2, total: 3),
            2: Self.page(["c"], next: nil, total: 3),
        ]
        let model = AnalyticsModel(service: service)
        await model.refresh()
        #expect(model.people.map(\.id) == ["a", "b"])
        #expect(model.peopleTotal == 3)
        #expect(model.hasMorePeople)
        #expect(model.sorts.map(\.id) == ["active", "joined"])

        await model.loadMorePeople()
        #expect(model.people.map(\.id) == ["a", "b", "c"])
        #expect(!model.hasMorePeople)

        // The end of the list is the end: nothing more is asked for.
        await model.loadMorePeople()
        #expect(service.pageCalls.map(\.cursor) == [0, 2])
    }

    @Test func aNewOrderStartsTheListOver() async {
        let service = Service()
        service.answer = .success(document())
        service.pages = [0: Self.page(["a", "b"], next: 2, total: 3)]
        let model = AnalyticsModel(service: service)
        await model.refresh()
        await model.loadMorePeople()

        service.pages = [0: Self.page(["c"], next: nil, total: 3)]
        await model.choosePeople(sort: "joined")
        #expect(model.sort == "joined")
        #expect(model.people.map(\.id) == ["c"])
        #expect(service.pageCalls.last?.sort == "joined")
        #expect(service.pageCalls.last?.cursor == 0)
    }

    // A nightly run landing mid-scroll reorders the ranks, so pages from the
    // older rollup are dropped instead of stitched onto the new order.
    @Test func aRollupWrittenMidScrollRestartsTheList() async {
        let service = Service()
        service.answer = .success(document())
        service.pages = [0: Self.page(["a", "b"], next: 2, total: 4)]
        let model = AnalyticsModel(service: service)
        await model.refresh()

        service.pages = [
            0: Self.page(["x", "y"], next: 2, total: 4, generatedAt: "2026-08-22T04:00:00.000Z"),
            2: Self.page(["z"], next: nil, total: 4, generatedAt: "2026-08-22T04:00:00.000Z"),
        ]
        await model.loadMorePeople()
        #expect(model.people.map(\.id) == ["x", "y"])
        #expect(model.hasMorePeople)
    }

    @Test func aFailedPageKeepsTheRowsAlreadyLoaded() async {
        let service = Service()
        service.answer = .success(document())
        service.pages = [0: Self.page(["a", "b"], next: 2, total: 3)]
        let model = AnalyticsModel(service: service)
        await model.refresh()
        service.pageFailure = CloudSyncError.unreachable(.timedOut, code: -1001)
        await model.loadMorePeople()
        #expect(model.people.map(\.id) == ["a", "b"])
        #expect(model.peopleError?.contains("-1001") == true)
    }
}
